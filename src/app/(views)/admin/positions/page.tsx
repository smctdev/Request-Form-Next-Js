...
const SetupPosition: React.FC = () => {
  ...
  const handleDelete = async (toDeleteId: number) => {
    ...
    setIsRefresh(true);
    try {
      const response = await api.delete(`/delete-position/${toDeleteId}`);
      if (response.status === 200) {
        Swal.fire({
          icon: "success",
          ...
          <div className=" p-4  w-1/2 md:w-1/3 bg-base-100 flex flex-col justify-center rounded-[12px] shadow-lg">
            ...
            <div className="flex justify-center space-x-2 md:justify-end">
              ...
              <button
                ...
                type="button"
                className="w-full py-2 text-white bg-red-500 border rounded-lg cursor-pointer md:w-auto md:px-4"

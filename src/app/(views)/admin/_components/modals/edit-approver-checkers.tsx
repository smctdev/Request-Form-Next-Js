...
export default function EditApproverCheckers({
  isOpenUpdate,
  ...
  useEffect(() => {
    if (!isOpenUpdate && !selectedApprover) return;

    setFormInputs({
      approver: String(selectedApprover?.id),
      checker: String(selectedApprover?.checkers[0]?.id || ""),
      checker_category: String(selectedApprover?.checker_category),
    });
  }, [isOpenUpdate, selectedApprover]);

  return (
    <dialog className="modal" open={isOpenUpdate}>
      <div className="modal-box">
        <h3 className="font-bold text-lg!">Edit Approver Checkers</h3>

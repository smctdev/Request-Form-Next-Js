...
const EditAreaManager = ({
  ...
  useEffect(() => {
    const fetchBranches = async () => {
      try {
        const response = await api.get(`/view-area-manager/${selectedUser.id}`);
  ...
  useEffect(() => {
    const fetchAreaManagerData = async () => {
      try {
        const response = await api.get(`/area-manager/${areaManagerId}`);
    ...
    if (selectedBranches.length > 0) {
      ...
      try {
        ...
        const response = await api.post(
          `/update-area-manager/${selectedUser.user_id}`,
          putData
        );
            ...
            <div className="flex flex-wrap px-4 mt-4 mb-4">
              {selectedBranches.map((branchId) => {
                ...
                return (
                  ...
                  >
                    <span className="text-white">{branch?.branch_code}</span>
                    <button
                      className="ml-2"
